...
import { getAllReviews, addReview, calculateReportStatistics, calculateAverageRatings, getAllCostReviews, addCostReview, calculateAverageCostRatings } from '../services/reviewService.ts';
...
import type { Review, BankInfo, BankConfig, CostReview } from '../types.ts';
                ...
                return `
                    <div class="bank-card" id="bank-${encodeURIComponent(status.name)}">
                        ...
                        <div class="buttons-container">
                            ...
                            <a href="/evaluate-cost/${encodeURIComponent(status.name)}" class="button button-secondary">Evaluar Costos</a>
                        </div>
...
/**
 * Muestra la página para evaluar los costos de un banco específico.
 */
export function showEvaluateCostPage(ctx: Context) {
    const { banco: encodedBankName } = helpers.getQuery(ctx, { mergeParams: true });
    const bankName = decodeURIComponent(encodedBankName || '');
    ...
    <main class="main-content">
        ...
        <div class="evaluation-card card">
            <div class="bank-header">
                <img src="${bankImage}" alt="${bankName} Logo" class="bank-logo">
                <div class="bank-info">
                    <h1>Evaluar Costos de ${bankName}</h1>
                    <p>Comparte tu opinión sobre los costos y tarifas del banco</p>
                </div>
            </div>

            <form class="evaluation-form" id="costEvaluationForm" action="/evaluate-cost/${encodeURIComponent(bankName)}" method="POST">
                <div class="form-group">
                    <label>¿Cómo calificarías el costo de los servicios?</label>
                    <div class="star-rating">
                        <div class="stars">
                            ${[1, 2, 3, 4, 5].map(n => `
                                <input type="radio" name="costRating" value="${n}" id="costRating${n}" required hidden>
                                <label for="costRating${n}" class="star" data-rating="${n}">
                                    <i class="fas fa-star"></i>
                                </label>
                            `).join('')}
                        </div>
                        <div class="rating-text">Selecciona una calificación (1 = Muy costoso, 5 = Muy accesible)</div>
                    </div>
                </div>

                <div class="form-group">
                    <label>Tipo de tarifas evaluadas</label>
                    <div class="category-tags">
                        ${[
                            { value: 'mantenimiento', text: 'Mantenimiento de Cuenta' },
                            { value: 'transferencias', text: 'Transferencias' },
                            { value: 'tarjetas', text: 'Tarjetas de Crédito/Débito' },
                            { value: 'prestamos', text: 'Préstamos' },
                            { value: 'cajeros', text: 'Uso de Cajeros' }
                        ].map(type => `
                            <input type="checkbox" name="feeType" value="${type.value}" id="fee${type.value}" hidden>
                            <label for="fee${type.value}" class="category-tag">${type.text}</label>
                        `).join('')}
                    </div>
                </div>

                <div class="form-group">
                    <label for="affordabilityComment">Describe tu experiencia con los costos</label>
                    <textarea
                        id="affordabilityComment"
                        name="affordabilityComment"
                        class="form-control"
                        placeholder="Describe los costos y cómo afectan tu experiencia como cliente..."
                        required
                    ></textarea>
                </div>

                <div class="form-group">
                    <label for="comparisonComment">Comparación con otros bancos (opcional)</label>
                    <textarea
                        id="comparisonComment"
                        name="comparisonComment"
                        class="form-control"
                        placeholder="¿Cómo comparan los costos de este banco con otros?"
                    ></textarea>
                </div>

                <div class="buttons-container">
                    <button type="submit" class="button">Enviar Evaluación</button>
                    <a href="/status" class="button button-secondary">Cancelar</a>
                </div>
            </form>
            <div class="success-message" id="successMessage" style="display: none;">
                ¡Evaluación de costos enviada con éxito!
            </div>
        </div>
    </main>
    `;
    ...
    ctx.response.body = renderPage({
        title: `Evaluar Costos de ${bankName}`,
        bodyContent,
        styles: ['/css/evaluate.css'],
        scripts: ['/js/evaluate-cost.js']
    });
}

/**
 * Procesa el envío del formulario de evaluación de costos.
 */
export async function handleEvaluateCostSubmit(ctx: Context) {
    try {
        const { banco: encodedBankName } = helpers.getQuery(ctx, { mergeParams: true });
        const bankName = decodeURIComponent(encodedBankName || '');
        ...
        const formData = await body.value;
        const costRatingValue = formData.get("costRating");
        const affordabilityCommentValue = formData.get("affordabilityComment");
        const feeTypeValues = formData.getAll("feeType");
        const comparisonCommentValue = formData.get("comparisonComment");

        if (!costRatingValue || !affordabilityCommentValue) {
            ctx.response.status = 400;
            ctx.response.body = { message: "La calificación y el comentario son requeridos" };
            return;
        }
        ...
        const reviewData = {
            banco: bankName,
            costRating: costRatingValue,
            affordabilityComment: affordabilityCommentValue,
            feeType: feeTypeValues,
            comparisonComment: comparisonCommentValue
        };

        await addCostReview(reviewData);

        ctx.response.redirect(`/thank-you-cost?banco=${encodeURIComponent(bankName)}`);

    } catch (error) {
        console.error("Error processing cost evaluation submission:", error);
        ctx.response.status = 500;
        ctx.response.body = renderPage({
             title: "Error",
             bodyContent: `<main class='main-content'><h1>Error al procesar la evaluación de costos</h1><p>${error.message}</p></main>`,
             styles: ['/css/error.css']
         });
    }
}
...
/**
 * Muestra la página de agradecimiento por la evaluación de costos.
 */
export function showThankYouCostPage(ctx: Context) {
    const { banco: encodedBankName } = helpers.getQuery(ctx, { mergeParams: true });
    const bankName = decodeURIComponent(encodedBankName || 'este banco');
    ...
    <main class="main-content success-card">
        ...
        <h1 class="success-title">¡Gracias por tu evaluación de costos!</h1>
        <p class="success-message">
            Tu opinión sobre los costos de ${bankName} ha sido registrada. Ayuda a otros usuarios a tomar decisiones financieras más informadas.
        </p>
        <a href="/status" class="button">Volver a Estados</a>
        <p class="redirect-text">
            Serás redirigido automáticamente en <span id="countdown" class="countdown">5</span> segundos...
    ...
    ctx.response.body = renderPage({
        title: "Evaluación de Costos Enviada",
        bodyContent,
        styles: ['/css/thank-you.css'],
        scripts: ['/js/thank-you.js']
    });
