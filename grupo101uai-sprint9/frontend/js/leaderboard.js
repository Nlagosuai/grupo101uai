...
/**
 * Obtiene el estilo CSS en línea para el valor de la puntuación según su valor exacto
 */
function getScoreValueStyle(score) {
    if (score <= 1) return 'color: #e74c3c; font-weight: 800;'; // Rojo fuerte para 1 o menos
    if (score <= 2) return 'color: #e67e22; font-weight: 700;'; // Naranja para 2 o menos
    if (score <= 3) return 'color: #f1c40f; font-weight: 600;'; // Amarillo para 3 o menos
    if (score <= 4) return 'color: #2ecc71; font-weight: 600;'; // Verde para 4 o menos
    if (score <= 5) return 'color: #27ae60; font-weight: 700;'; // Verde oscuro para 5
    return '';
}
...
function showDetails(bankData) {
    ...
    overallItem.className = `score-item ${hasOverallScore ? getScoreClass(bankData.overall) : 'score-no-data'}`;
    ...
    categories.forEach(category => {
        ...
        scoreItem.className = `score-item ${hasScore ? getScoreClass(score) : 'score-no-data'}`;
