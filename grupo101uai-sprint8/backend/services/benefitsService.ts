...
/**
 * Obtiene los beneficios de un banco específico
 */
export async function getBenefitsByBank(bankName: string): Promise<Benefit[]> {
  const allBenefits = await getAllBenefits();
  return allBenefits.filter(benefit => benefit.bank.toLowerCase() === bankName.toLowerCase());
}
